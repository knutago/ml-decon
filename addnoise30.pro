pro addnoise30,filein,band,exptime,nexp,fileout
;; read image
...
if band eq 'J' then scale=678536.*10^(-0.4*(25.-15))*exptime
if band eq 'H' then scale=807529.*10^(-0.4*(25.-15))*exptime
if band eq 'K' then scale=430284.*10^(-0.4*(25.-15))*exptime
...
if band eq 'J' then skyc=3.6*exptime	;;; from Bob's spreadsheet
if band eq 'H' then skyc=(29.6+0.0)*exptime	;;; from Bob's spreadsheet
if band eq 'K' then skyc=(22.8+22.2)*exptime	;;; from Bob's spreadsheet
