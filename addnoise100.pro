pro addnoise100,filein,band,exptime,nexp,fileout
;; read image
...
if band eq 'J' then scale=7539294.*10^(-0.4*(25.-15))*exptime
if band eq 'H' then scale=8972542.*10^(-0.4*(25.-15))*exptime
if band eq 'K' then scale=4780934.*10^(-0.4*(25.-15))*exptime
...
if band eq 'J' then skyc=4.2*exptime	;;; from Bob's spreadsheet
if band eq 'H' then skyc=(34.7+0.0)*exptime	;;; from Bob's spreadsheet
if band eq 'K' then skyc=(26.8+26.0)*exptime	;;; from Bob's spreadsheet
